 ...
 * {@link org.springframework.data.relational.core.mapping.NamingStrategy}.
 *
 * @author Mark Paluch
 * @author Kurt Niemi
 * @since 2.0
 */
class OverrideDerivedSqlIdentifier implements SqlIdentifier {

	private final String name;
	private final boolean quoted;

	OverrideDerivedSqlIdentifier(String name, boolean quoted) {
	...
	public SqlIdentifier transform(UnaryOperator<String> transformationFunction) {
		...
		return new OverrideDerivedSqlIdentifier(transformationFunction.apply(name), quoted);
	}
