...
public class HomeMadeCalculationTasklet implements Tasklet {
    ...
    private void initHomeMade() {
        ...
        // 自制件列表信息增加待计算成本列
        homeMades.addColumns(DoubleColumn.create("material_cost"));
        homeMades.addColumns(DoubleColumn.create("manpower_cost"));
        homeMades.addColumns(DoubleColumn.create("manufacture_cost"));
        homeMades.addColumns(DoubleColumn.create("total_cost"));
        homeMades.addColumns(DoubleColumn.create("init_total_cost"));
        homeMades.addColumns(DoubleColumn.create("init_total_quantity"));
        homeMades.doubleColumn("manpower_cost").fillWith(RandomUtils.nextDouble(0.0, 100.00));
        homeMades.doubleColumn("manufacture_cost").fillWith(RandomUtils.nextDouble(0.0, 100.00));
        homeMades.doubleColumn("init_total_cost").fillWith(RandomUtils.nextDouble(0.0, 100.00));
        homeMades.doubleColumn("init_total_quantity").fillWith(RandomUtils.nextDouble(0.0, 100.00));
    ...
    private void validateData() {
        Set<String> unPricedInventoryIds = materials
                ...
                .where(materials.doubleColumn("price").isMissing())
                .asSet();
        ...
        Assert.state(difference.size() == 0, "物料表中没有单价的自制件，在本月中未发现生产登数,请补充期初单价。相关货品ID:" + difference);
    }
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        homeMades.forEach(row -> {
            ...
            if (row.isMissing("total_cost")) {
                ...
                DoubleColumn quantityCol = materialTable.doubleColumn("quantity");// 实际使用数量
                DoubleColumn priceCol = materialTable.doubleColumn("price");
                DoubleColumn amountCol = quantityCol.multiply(priceCol);
                row.setDouble("material_cost", amountCol.sum());
                double totalCost = row.getDouble("manpower_cost") + row.getDouble("manufacture_cost") + row.getDouble("material_cost");
                row.setDouble("total_cost", totalCost);
            }
        });

        // 针对自制件列表按货品id分组，将所有算过成本的算出单价
        Table inventoryTable = homeMades
                .summarize(List.of("inventory_id", "total_cost", "quantity", "init_total_cost", "init_total_quantity"), AggregateFunctions.countNonMissing, AggregateFunctions.sum, AggregateFunctions.first)
                .by("inventory_id");
        // 找到已经计算完成本的货品列表
        Table completedTable = inventoryTable
                .where(inventoryTable.doubleColumn("Count [total_cost]").isEqualTo(inventoryTable.doubleColumn("Count [inventory_id]")));
        // 把已计算完的货品列表计算出单价
        DoubleColumn finalCost = completedTable.doubleColumn("Sum [total_cost]").add(completedTable.doubleColumn("First [init_total_cost]"));
        DoubleColumn finalQuantity = completedTable.doubleColumn("Sum [quantity]").add(completedTable.doubleColumn("First [init_total_quantity]"));
        DoubleColumn priceColumn = finalCost.divide(finalQuantity).setName("price");
        completedTable.addColumns(priceColumn);
        // 最终结果: 半成品加权平均后的单价
        Map<String, Double> invPriceMap = new HashMap<>();
        completedTable.forEach(row -> invPriceMap.put(row.getString("inventory_id"), row.getDouble("price")));
        ...
        if (homeMades.column("total_cost").countMissing() > 0) {
            return RepeatStatus.CONTINUABLE;
        } else {
            // 自制件每行计算自身的财务单价
            DoubleColumn priceCol = homeMades.doubleColumn("total_cost")
                    .divide(homeMades.doubleColumn("quantity"))
                    .setName("price");
            homeMades.addColumns(priceCol);
            System.out.println(homeMades);
            System.out.println(materials);
            System.out.println(completedTable);
            // TODO 写入结果
