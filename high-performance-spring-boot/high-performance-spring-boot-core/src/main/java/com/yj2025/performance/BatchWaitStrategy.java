...
//            Thread.sleep(500);
    ...
    public class TimerThread extends Thread {
        ...
        private AtomicLong timeMillis = new AtomicLong(maxWaitSeconds * 1000);

        public TimerThread(SequenceBarrier barrier) {
            this.barrier = barrier;
        }

        /**
         * TODO 是否考虑当没有再开始倒计时的时候，把当前守护线程暂停， 虽然已经做了当批处理shutdown的时候，关闭计时器。如果不关闭呢？常驻内存？会不会占用cpu？
         */
        public void countDown() {
            timeMillis.set(maxWaitSeconds * 1000);
//            synchronized (this) {
//                this.resume();
//            }
        }
        ...
        public void run() {
            while (true) {
                if (timeMillis.get() != 0L) {
                    timeMillis.decrementAndGet();
                    try {
                        Thread.sleep(1);
